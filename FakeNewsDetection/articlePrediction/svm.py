...
def Svm(text):
    #Importing dataset
    true_data = pd.read_csv(os.path.join(base_location,'processedTrue.csv'))
    fake_data = pd.read_csv(os.path.join(base_location,'processedFake.csv'))
    ...
    with open(f'{base_location}\models\model.pkl', 'rb') as file:
